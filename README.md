...
<ins>**Note**</ins>: You have selected the ``feature/minseglen`` branch. This branch includes the argument ``minSegmentLength`` in ``getChangepoints()``, which passes to the ``minseglen`` argument in  ``cpt.mean()`` from the ``changepoint`` package for PELT. This was added by request, but must be used with caution. ASCEPT may not perform as expected when using this feature as when using the official code in the ``master`` branch. If you do not need this feature, we recommend using the ``master`` branch instead.
